...
<img width="90%"  alt="NCA for shape classification." src="https://github.com/user-attachments/assets/956c07df-0bbc-4588-a468-bfdc2f1603bc" />
 <figcaption><sub><Strong>Neural cellular automata for shape classification.</Strong> (A) Cellular brick module. (B) Examples of cellular bricks assembled into four different shapes. (C) Cells take as input local information from their connecting neighboring cells and their hidden channels. Information is aggregated locally, enabling the object to recognize its particular shape over multiple iterations. (D) The local update rules are encoded with a neural cellular automata, a deep neural network.</sub> </figcaption>
</p>
...
We validate our approach on real hardware,  demonstrating that shape classification is not only feasible in a fully distributed setting, but also robust under the kinds of imperfections and hardware constraints that arise in the physical world.  By enabling modular cellular bricks to autonomously infer their morphology and detect structural faults, this system brings us closer to creating artificial systems with some of the properties that helped biological organisms to thrive — such as decentralized self-assessment, resilience to damage, and adaptive organization — without the need for centralized control or predefined templates. 
...
<p align="center">
<img width="80%" alt="Robustness to faulty cells" src="https://github.com/user-attachments/assets/5f609e47-9496-4e21-b9f6-94e9f4df45ba" /><figcaption><sub>For two of the shapes assembled in hardware (guitar and plane), we disabled particular cells (marked in red) to test the model's robustness. Because of their design, the plane
...
For each training iteration, we sampled random shapes, damaged them, and ran the NCA for a random number of steps (between 64 and 96). The last channels for each cell were treated as logits for classification, and the model was optimized using the labels generated by the synthetically damaged shapes.  We achieved a damage detection accuracy of more than 90% for all the different types of shapes it was tested on. 
...
To test the ability of our system to guide damage recovery, we started from a small cluster of cells, and added cells in the direction determined by the existing cells; this process was repeated until no more damage was detected.
...
Surprisingly, without ever being trained with only a few cells, the model was able to detect damage and recover almost all shapes across all object classes with high accuracy. We compared the approach across different cell hidden dimension sizes and found that models with larger hidden states performed significantly better. These results are likely explained by the fact that the larger hidden states allow the models to capture more information across development. 
...
Want to learn more about self-organizing AI and collective systems? Check out this [blog post](https://sebastianrisi.com/self_assembling_ai/) and this [survey article](https://arxiv.org/abs/2111.14377) of recent developments in collective intelligence for deep learning. 
